...
ImageFile.LOAD_TRUNCATED_IMAGES = True
 ...
 '''
    该文件旨在对UI设计的截图进行图片处理，主要包括：
1.转储XML运行时代码：XML 运行时代码包含有关当前UI的所有信息，
包括所有包含的组件及其相应的边界、类、文本、关 于可执行性（如可检查、可点击和可滚动）的布尔属性，
以及它们之间的层次关系；
2.删除重复的数据：比较GUI组件序列的哈希值，将当前转储的XML代码文件与收集到的数据进行比较
    '''


def checkFileValidity(inputFile):
    '''
    检查XML文件的有效性，如果可能，则忽略他；
    由于未知的原因，一些XML文件的内容是重复的
    '''
    homeScreen_list = ["Make yourself at home", "You can put your favorite apps here.",
                       "To see all your apps, touch the circle."]
    unlockHomeScreen_list = ["Camera", "[16,600][144,728]", "Phone", "[150,1114][225,1189]", "People",
                             "[256,1114][331,1189]", "Messaging", "[468,1114][543,1189]", "Browser",
                             "[574,1114][649,1189]"]
    with open(inputFile) as f:
        ...
        if 'bounds="[0,0][800,1216]"' in content and '<hierarchy rotation="1">' not in content:
            if not all(keyword in content for keyword in homeScreen_list) and not all(
                    keyword in content for keyword in unlockHomeScreen_list):
...
def compareHisto(first, sec):
    ...
    try:
        ...
        print
        "First FIle: %s is wrong" % first
        return 'error1'
    try:
        imB = Image.open(open(sec, 'rb'))
    except IOError:
        print
        "Second FIle: %s is wrong" % sec
        return 'error2'
    # 标准化图片格式
...
def imageProcessing(img):
    ...
    对图片进行基础处理
    """
    img = img.crop((0, 32, 799, 1214))  # 裁剪边框
    img = img.resize((256, 256), Image.ANTIALIAS)  # 修改大小
    return img
...
def drawImageLabel(inputFile_xml, fileHash, directory, specificOutputDir):
    ...
    def imageWF(draw, dimensions, base):
        ...
        draw.line((dimensions['from'][0], dimensions['to'][1], dimensions['to'][0], dimensions['from'][1]),
                  fill="#ffffff", width=2)
    ...
    def textViewWF(draw, dimensions, base):
        ...
        ### "#00ff00":Green
        draw.rectangle(
            (tuple(numpy.add(dimensions['from'], (10, 10))), tuple(numpy.subtract(dimensions['to'], (10, 10)))),
            fill="#00ff00")
    ...
    def switchWF(draw, dimensions, base):
        ...
        switch = {}
        switch["from"] = tuple(
            (dimensions["from"][0] + ((dimensions["to"][0] - dimensions["from"][0]) / 2), dimensions["from"][1]))
    ...
    def videoViewWF(draw, dimensions, base):
        im = Image.open('UIelement/videoview.png').convert('RGBA').resize(
            (dimensions['to'][0] - dimensions['from'][0], dimensions['to'][1] - dimensions['from'][1]))
        base.paste(im, (dimensions['from']))

    def spinnerWF(draw, dimensions, base):
        im = Image.open('UIelement/spinner.png').convert('RGBA').resize(
            (dimensions['to'][0] - dimensions['from'][0], dimensions['to'][1] - dimensions['from'][1]))
        base.paste(im, (dimensions['from']))
    ...
    def chronometerWF(draw, dimensions, base):
        im = Image.open('UIelement/chronometer.png').convert('RGBA').resize(
            (dimensions['to'][0] - dimensions['from'][0], dimensions['to'][1] - dimensions['from'][1]))
        base.paste(im, (dimensions['from']))
    ...
    widgets_set = set(
        ['ImageView', 'ImageButton', 'EditText', 'Button', 'TextView', 'ToggleButton', 'CheckedTextView', 'View',
         'CheckBox', 'RadioButton', 'ProgressBar', 'Switch', 'SeekBar', 'WebView', 'CompoundButton',
         'MultiAutoCompleteTextView', 'VideoView', 'Spinner', 'RatingBar', 'Chronometer'])
    ...
    if len(widgetInfo_list) > 2:
        widgetInfoSorted_list = sorted(widgetInfo_list, key=operator.itemgetter(2),
                                       reverse=True)  # sort the widgets by the area in descent
        for (widgetName, coordinates, area) in widgetInfoSorted_list:
            if widgetName in widgets_set:
                ...
                draw.rectangle((coordinates["from"], coordinates["to"]),
                               fill="#dddddd")  # draw other infrequent widgets
        ...
        if not os.path.exists(os.path.join(specificOutputDir, directory)):
            ...
            fileHash = str(a) + str("!") + str(b) + str("!") + str(c) + str("!") + str(d) + str("!") + str(e) + str(
                "!") + str(f) + str("!") + str(g) + str("!") + str(h) + str("!") + fileHash
            source_img.save(
                os.path.join(os.path.join(os.path.join(specificOutputDir, directory), "source"), fileHash + ".png"),
                "PNG")
            target_img.save(
                os.path.join(os.path.join(os.path.join(specificOutputDir, directory), "target"), fileHash + ".png"),
                "PNG")
                        ...
                        if os.path.isfile(imageDir):
                            ...
                            if fileHash not in hash_dic:
                                ...
                                try:
                                    ...
                                    e
                print
                "For %s, %s are valid " % (packageName, countValid)
            else:
                print
                "Something wrong with %s" % (packageName), len(line_list)
...
def multiThreadImageCollection(inputDir_list, outputFile):
    ...
    print
    "totally %s directory" % len(dir_list)
    pool = Pool(8)
    results = pool.map(collectFuntionalityWirify, dir_list)
    pool.close()
    pool.join()
    print
    "There are %s apps" % (len(results))
...
if __name__ == '__main__':
    ...
    try:
        ...
        multiThreadImageCollection(["/media/cheer/New Volume/5K_data/verbo1", "/media/cheer/New Volume/5K_data/verbo2",
                                    "/media/cheer/New Volume/5K_data/verbo3"], log_path)
